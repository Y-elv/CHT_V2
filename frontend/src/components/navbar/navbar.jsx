...
const Navbar = ({active}) => {
  ...
  return (
    <nav
      className="navbar navbar-expand-lg navbar-light bg-[#B8C2D7] px-10 py-2"
    ...
    >
      <div className="container-fluid relative">
        ...
        "tablet-menu translate-x-[0px] h-[100vh] absolute top-2 right-[-40px] p-10 px-24 z-3 bg-white transition-all border border-gray-400 flex items-start justify-start flex-col":
          ...
          <Link className="nav-link text-black" to="/updates">Updates</Link>
          <Link className="nav-link text-black" to="/our-team" style={{color:active=="ourTeam"?"orange":"red"}}>Our Team</Link>
          <Link className="nav-link text-black" to="/menu">Menu</Link>
        </div>
        
        <div className="links flex flex-col items-start justify-start  gap-1 my-2">
        <Link to="/login" className="px-4 py-2 w-full bg-orange-400 hover:bg-orange-600 rounded-md">Login</Link>
        <Link  to="/register" className="px-4 py-2 w-full bg-blue-400 hover:bg-blue-600 rounded-md">Signup</Link>
        </div>
        ...
        <div className=" mx-3 hidden tablet:flex flex-row items-center justify-center gap-3">
          <Link to="/login" className="px-4 py-2 bg-orange-400 hover:bg-orange-600 rounded-full">Login</Link>
          <Link to="/register" className="px-4 py-2 bg-blue-400 hover:bg-blue-600 rounded-full">Signup</Link>
        </div>
